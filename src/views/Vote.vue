<template>
  <div class="mx-auto items-center justify-center p-8 text-left">
    ...
    <div class="flex flex-col sm:flex-row">
      ...
      <div
        class="align-right inline-block bg-white px-6 px-8 py-2.5 text-black"
      >
        ...
        <span v-else-if="isVoting">
          <i class="material-icons text-op-teal mr-2 animate-spin text-lg"
            >schedule</i
    ...
    <div
      class="grid w-screen grid-cols-1 place-content-center items-center gap-2 sm:grid-cols-3"
    >
      <div v-for="option in selections" :key="option.key" class="px-2 py-6">
        ...
        <button
          v-if="isVoted"
          class="inline-block w-32 rounded bg-white px-6 py-2.5 leading-tight text-gray-500 shadow-md"
        >
          {{ $t("vote.button_voted") }}
        </button>
        <button
          v-else-if="!isVoteReady"
          class="inline-block w-32 rounded bg-white px-6 py-2.5 leading-tight text-gray-500 shadow-md"
        >
        ...
        <button
          ...
          class="inline-block w-32 rounded bg-green-500 px-6 py-2.5 leading-tight text-white shadow-md"
        >
...
export default defineComponent({
  ...
  setup() {
    ...
    const updateUserStatus = async () => {
      ...
      try {
        const docu = await getDoc(
          doc(db, `users/${store.state.user.uid}/private/votes`)
        );
        if (docu.exists()) {
    ...
    selections.value = Object.assign([], vote_event.selections).map(
      (a: { id: number; key: string }) => {
    ...
    const totalcount = computed(() =>
      selections.value.map((c) => c.count).reduce((p, c) => p + c)
    );
    ...
    const isVoteReady = computed(
      () =>
        ...
        !isVoting.value &&
        !isVoted.value &&
        vote_event.start < new Date() &&
        new Date() < vote_event.end &&
